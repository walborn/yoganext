...
const hhmm = minutes => (mm => `${`0${(minutes - mm) / 60}`.slice(-2)}:${`0${mm}`.slice(-2)}`)(minutes % 60);

const fetchData = async () => {
    const response = await fetch('https://om-rest.herokuapp.com/lessons?sort=day&offset=0&limit=100'); // hidden:false
...
export default class Schedule extends React.Component {
    ...
    static Master = ({ className, alternate, master }) => {
